...
function buildKwinScript(
  ...
  const activateSnippet = activatePoe
    ...
    : "";
  // When the overlay OPENS, Electron can't give its own InputProxy window
  // keyboard focus on Wayland, so the user had to alt-tab to it. On the pause
  // reload, activate the InputProxy (the tiny ~1x1 EE2 window) from the
  // compositor side so the price-check panel takes focus immediately.
  const overlayActivateSnippet = activateOverlay
    ? `(function () {
  var ol = workspace.windowList ? workspace.windowList() : workspace.clientList();
  for (var k = 0; k < ol.length; k++) {
    var ow = ol[k];
    if (ow && isOverlayWindow(ow) && ow.frameGeometry &&
        (ow.frameGeometry.width | 0) <= 20 && (ow.frameGeometry.height | 0) <= 20) {
      try { workspace.activeWindow = ow; } catch (e) {}
      break;
    }
  }
})();`
...
// On the overlay-open (pause) reload, focus the InputProxy from the compositor.
${overlayActivateSnippet}
...
export class WaylandTracker extends EventEmitter {
  ...
  private async _loadKwinScript(
    ...
    const next = this._loadChain
      .catch(() => {})
      .then(() => this._doLoadKwinScript(shortcuts, activatePoe, activateOverlay));
    this._loadChain = next.catch(() => {});
    return next;
  }

  private async _doLoadKwinScript(
    shortcuts: string[],
    activatePoe = false,
    activateOverlay = false,
  ): Promise<void> {
